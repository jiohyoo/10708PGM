...
%D=n by Ltot matrix
function [gradbeta gradbetad gradtheta gradalpha1 ]= fgradlhoodv5_PGM(beta,betad,theta,alpha1,D,X,L,n,p,q)
%%
% Ltot=sum(L);
% Lsum=[0;cumsum(L)];
...
% gradalpha1=-diag(betad)*sum(X-Xbeta-Dtheta,1)'+diag(betad)*alpha1*n; <- originally commented out
...
% gradbetad=-1./(2*betad)+1/2*sum(res.^2,1)'+diag((-res)'*(Xbeta+Dtheta));  <- originally commented out
gradbeta=gradbeta/n;
