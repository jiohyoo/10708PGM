function opt = syntheticExp2_test_CV_fn(ToyData, lambda_seq, kcv)
...
%         [prederr_tmp] = test_predict(theta, alpha1, beta, betad, Y_te_CV, D_te_CV);
            prederr_tmp = out;
...
smoothF= @(x)lhoodTfocsv5(x,D_tr,X_tr,Y_tr,L,n_tr,p,q);
nonsmoothH=@(varargin) tfocsProxGroupv6(opt_lambda,L,n_tr,p,q, varargin{:} ); % only returns value of nonsmooth

[xopt, out, opts] = pnopt(smoothF, nonsmoothH, x);
[beta, betad, theta, phi, alpha1, alpha2]= vecToParamv5(xopt,L,n_tr,p,q);
