function opt = syntheticExp2_PGM_CV_fn_0326(ToyData, lambda_seq, M)
...
[TRAIN,TEST] = crossvalind('LeaveMOut', n_tr , M);
...
for lam_idx = 1 : length(lambda_seq)
    ...
    for cv_idx = 1: kcv
		...
		X_tr_CV = X_tr(TEST ~= cv_idx, :); 
		D_tr_CV = D_tr(TEST ~= cv_idx, :); 
		X_te_CV  = X_tr(TEST == cv_idx, :);
        Y_te_CV  = Y_tr(TEST == cv_idx, :); 
		D_te_CV  = D_tr(TEST == cv_idx, :); 
